...
Goal: Convert BANKACCT.cob plus CUSTOMERS.DAT and TRANSACTIONS.DAT into a runnable Angular (standalone components) + FastAPI app with JSON persistence and tests. COBOL is authoritative for behavior even when it conflicts with skill defaults. Also convert MONTHEND.cob in a scheduled task in the scheduling framework suggested by the skill (APScheduler, with Configuration UI in the interactive application)

Inputs and discovery
- Inventory BANKACCT.cob (Interactive App), MONTHEND.cob (Batch job), CUSTOMERS.DAT, TRANSACTIONS.DAT.
